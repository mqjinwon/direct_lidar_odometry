...
dlo::OdomNode::OdomNode() : Node("dlo_odom_node") {
    ...
    this->icp_cb_group = this->create_callback_group(
        rclcpp::CallbackGroupType::MutuallyExclusive);
    auto icp_sub_opt = rclcpp::SubscriptionOptions();
    this->icp_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "pointcloud", 1,
        std::bind(&dlo::OdomNode::icpCB, this, std::placeholders::_1),
        icp_sub_opt);

    this->imu_cb_group = this->create_callback_group(
        rclcpp::CallbackGroupType::MutuallyExclusive);
    ...
    this->keyframe_pub =
        this->create_publisher<sensor_msgs::msg::PointCloud2>("keyframe", 1);

    this->save_traj_cb_group = this->create_callback_group(
        rclcpp::CallbackGroupType::MutuallyExclusive);
    this->save_traj_srv =
        this->create_service<direct_lidar_odometry::srv::SaveTraj>(
            "save_traj",
            std::bind(&dlo::OdomNode::saveTrajectory, this,
                      std::placeholders::_1, std::placeholders::_2),
            rmw_qos_profile_services_default, this->save_traj_cb_group);
    ...
    this->vf_scan.setLeafSize(this->vf_scan_res_, this->vf_scan_res_,
                              this->vf_scan_res_);
    this->vf_submap.setLeafSize(this->vf_submap_res_, this->vf_submap_res_,
                                this->vf_submap_res_);
...
void dlo::OdomNode::getParams() {
    // Version
    dlo::declare_param(this, "dlo/version", this->version_, "1.4.3");

    // Frames
    dlo::declare_param(this, "dlo/odomNode/odom_frame", this->odom_frame,
                       "odom");
    dlo::declare_param(this, "dlo/odomNode/child_frame", this->child_frame,
                       "base_link");
    ...
    // Gravity alignment
    dlo::declare_param(this, "dlo/gravityAlign", this->gravity_align_, true);

    // Keyframe Threshold
    dlo::declare_param(this, "dlo/odomNode/keyframe/threshD",
                       this->keyframe_thresh_dist_, 5.0);
    dlo::declare_param(this, "dlo/odomNode/keyframe/threshR",
                       this->keyframe_thresh_rot_, 45.0);

    // Submap
    dlo::declare_param(this, "dlo/odomNode/submap/keyframe/knn",
                       this->submap_knn_, 10);
    dlo::declare_param(this, "dlo/odomNode/submap/keyframe/kcv",
                       this->submap_kcv_, 10);
    dlo::declare_param(this, "dlo/odomNode/submap/keyframe/kcc",
                       this->submap_kcc_, 10);

    // Initial Position
    dlo::declare_param(this, "dlo/odomNode/initialPose/use",
                       this->initial_pose_use_, false);
    ...
    // Crop Box Filter
    dlo::declare_param(this, "dlo/odomNode/preprocessing/cropBoxFilter/use",
                       this->crop_use_, true);
    dlo::declare_param(this, "dlo/odomNode/preprocessing/cropBoxFilter/size",
                       this->crop_size_, 1.0);

    // Voxel Grid Filter
    dlo::declare_param(this, "dlo/odomNode/preprocessing/voxelFilter/scan/use",
                       this->vf_scan_use_, true);
    dlo::declare_param(this, "dlo/odomNode/preprocessing/voxelFilter/scan/res",
                       this->vf_scan_res_, 0.25);
    dlo::declare_param(this,
                       "dlo/odomNode/preprocessing/voxelFilter/submap/use",
                       this->vf_submap_use_, true);
    dlo::declare_param(this,
                       "dlo/odomNode/preprocessing/voxelFilter/submap/res",
                       this->vf_submap_res_, 0.5);

    // Adaptive Parameters
    dlo::declare_param(this, "dlo/adaptiveParams", this->adaptive_params_use_,
                       true);

    // IMU
    dlo::declare_param(this, "dlo/imu/use", this->imu_use_, true);
    dlo::declare_param(this, "dlo/odomNode/imu/calibTime",
                       this->imu_calib_time_, 3);
    dlo::declare_param(this, "dlo/odomNode/imu/bufferSize",
                       this->imu_buffer_size_, 2000);

    // GICP
    dlo::declare_param(this, "dlo/odomNode/gicp/minNumPoints",
                       this->gicp_min_num_points_, 10);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2s/kCorrespondences",
                       this->gicps2s_k_correspondences_, 10);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2s/maxCorrespondenceDistance",
                       this->gicps2s_max_corr_dist_,
                       1.0);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2s/maxIterations",
                       this->gicps2s_max_iter_, 32);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2s/transformationEpsilon",
                       this->gicps2s_transformation_ep_, 0.01);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2s/euclideanFitnessEpsilon",
                       this->gicps2s_euclidean_fitness_ep_, 0.01);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2s/ransac/iterations",
                       this->gicps2s_ransac_iter_, 5);
    dlo::declare_param(this,
                       "dlo/odomNode/gicp/s2s/ransac/outlierRejectionThresh",
                       this->gicps2s_ransac_inlier_thresh_, 1.0);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2m/kCorrespondences",
                       this->gicps2m_k_correspondences_, 20);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2m/maxCorrespondenceDistance",
                       this->gicps2m_max_corr_dist_, 0.5);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2m/maxIterations",
                       this->gicps2m_max_iter_, 32);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2m/transformationEpsilon",
                       this->gicps2m_transformation_ep_, 0.01);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2m/euclideanFitnessEpsilon",
                       this->gicps2m_euclidean_fitness_ep_, 0.01);
    dlo::declare_param(this, "dlo/odomNode/gicp/s2m/ransac/iterations",
                       this->gicps2m_ransac_iter_, 5);
    dlo::declare_param(this,
                       "dlo/odomNode/gicp/s2m/ransac/outlierRejectionThresh",
                       this->gicps2m_ransac_inlier_thresh_, 1.0);
}
...
void dlo::OdomNode::abortTimerCB() {
    if (abort_) {
        stop();
    }
}
...
void dlo::OdomNode::publishKeyframe() {
    ...
    if (this->keyframe_cloud->points.size() ==
        this->keyframe_cloud->width * this->keyframe_cloud->height) {
...
void dlo::OdomNode::initializeInputTarget() {
    ...
    pcl::PointCloud<PointType>::Ptr first_keyframe(
    ...
    this->keyframes.push_back(
        std::make_pair(std::make_pair(this->pose, this->rotq), first_keyframe));
    ...
    *this->keyframe_cloud = *first_keyframe;
    ...
    this->publish_keyframe_thread =
        std::thread(&dlo::OdomNode::publishKeyframe, this);
...
void dlo::OdomNode::gravityAlign() {
    ...
    // normalize
    double lin_norm = sqrt(pow(lin_accel[0], 2) + pow(lin_accel[1], 2) +
                           pow(lin_accel[2], 2));
    ...
    // normalize
    double grav_norm = sqrt(grav_q.w() * grav_q.w() + grav_q.x() * grav_q.x() +
...
void dlo::OdomNode::icpCB(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
    double then = rclcpp::Clock().now().seconds();
    ...
    this->comp_times.push_back(rclcpp::Clock().now().seconds() - then);
...
void dlo::OdomNode::imuCB(const sensor_msgs::msg::Imu::SharedPtr imu) {
    ...
    if (this->first_imu_time == 0.) {
        this->first_imu_time = imu->header.stamp.sec +
        ...
        if ((imu->header.stamp.sec - this->first_imu_time) <
            ...
            std::cout << "done" << std::endl;
            std::cout << "  Gyro biases [xyz]: " << this->imu_bias.gyro.x
                      << ", " << this->imu_bias.gyro.y << ", "
        ...
        this->imu_meas.stamp = imu->header.stamp.sec +
                               imu->header.stamp.nanosec * 1e-9;
...
void dlo::OdomNode::getNextPose() {
    ...
    // Both source and target clouds are in the global frame now, so
    // tranformation is global
    this->propagateS2M();
...
void dlo::OdomNode::integrateIMU() {
    ...
    for (uint32_t i = 0; i < imu_frame.size(); ++i) {
        ...
        q.w() -=
            0.5 *
            (qq.x() * imu_frame[i].ang_vel.x + qq.y() * imu_frame[i].ang_vel.y +
             qq.z() * imu_frame[i].ang_vel.z) *
            dt;
        q.x() +=
            0.5 *
            (qq.w() * imu_frame[i].ang_vel.x - qq.z() * imu_frame[i].ang_vel.y +
             qq.y() * imu_frame[i].ang_vel.z) *
            dt;
        q.y() +=
            0.5 *
            (qq.z() * imu_frame[i].ang_vel.x + qq.w() * imu_frame[i].ang_vel.y -
             qq.x() * imu_frame[i].ang_vel.z) *
            dt;
        q.z() +=
            0.5 *
            (qq.x() * imu_frame[i].ang_vel.y - qq.y() * imu_frame[i].ang_vel.x +
             qq.w() * imu_frame[i].ang_vel.z) *
            dt;
    }
...
void dlo::OdomNode::propagateS2M() {
    ...
    this->rotSO3 << this->T(0, 0), this->T(0, 1), this->T(0, 2), this->T(1, 0),
...
void dlo::OdomNode::computeSpaciousness() {
    ...
    for (int i = 0; i <= this->current_scan->points.size(); i++) {
        float d = std::sqrt(pow(this->current_scan->points[i].x, 2) +
                            pow(this->current_scan->points[i].y, 2) +
...
void dlo::OdomNode::updateKeyframes() {
    ...
    for (const auto& k : this->keyframes) {
        ...
        float delta_d = sqrt(pow(this->pose[0] - k.first.first[0], 2) +
                             pow(this->pose[1] - k.first.first[1], 2) +
    ...
    float dd = sqrt(pow(this->pose[0] - closest_pose[0], 2) +
                    pow(this->pose[1] - closest_pose[1], 2) +
    ...
    if (newKeyframe) {
        ...
        this->keyframes.push_back(std::make_pair(
            std::make_pair(this->pose, this->rotq), this->current_scan_t));

        // compute kdtree and keyframe normals (use gicp_s2s input source as
        // temporary storage because it will be overwritten by
        // setInputSources())
...
void dlo::OdomNode::setAdaptiveParams() {
    ...
    } else if (this->metrics.spaciousness.back() > 10.0 &&
               this->metrics.spaciousness.back() <= 20.0) {
        this->keyframe_thresh_dist_ = 5.0;
    } else if (this->metrics.spaciousness.back() > 5.0 &&
               this->metrics.spaciousness.back() <= 10.0) {
    ...
    if (this->submap_kf_idx_curr == this->submap_kf_idx_prev) {
        ...
        for (auto k : this->submap_kf_idx_curr) {
            ...
            *submap_cloud_ += *this->keyframes[k].second;
            ...
            this->submap_normals.insert(std::end(this->submap_normals),
...
bool dlo::OdomNode::saveTrajectory(
    std::shared_ptr<direct_lidar_odometry::srv::SaveTraj::Request> req,
    std::shared_ptr<direct_lidar_odometry::srv::SaveTraj::Response> res) {
    std::string kittipath = req->save_path + "/kitti_traj.txt";
    std::ofstream out_kitti(kittipath);

    std::cout << std::setprecision(2) << "Saving KITTI trajectory to "
              << kittipath << "... ";
    std::cout.flush();
    ...
    for (const auto& pose : this->trajectory) {
        ...
        out_kitti << std::fixed << std::setprecision(9) << R(0, 0) << " "
                  << R(0, 1) << " " << R(0, 2) << " " << t.x() << " " << R(1, 0)
                  << " " << R(1, 1) << " " << R(1, 2) << " " << t.y() << " "
...
void dlo::OdomNode::debug() {
    ...
    std::ifstream stat_stream(
        "/proc/self/stat", std::ios_base::in);  // get info from proc directory
    ...
    std::cout << "Position    [xyz]  :: " << this->pose[0] << " "
              << this->pose[1] << " " << this->pose[2] << std::endl;
    std::cout << "Orientation [wxyz] :: " << this->rotq.w() << " "
              << this->rotq.x() << " " << this->rotq.y() << " "
              << this->rotq.z() << std::endl;
    std::cout << "Distance Traveled  :: " << length_traversed << " meters"
              << std::endl;
    std::cout << "Distance to Origin :: "
              << sqrt(pow(this->pose[0] - this->origin[0], 2) +
                      pow(this->pose[1] - this->origin[1], 2) +
    ...
    std::cout << "Cores Utilized   :: " << std::setfill(' ') << std::setw(6)
              ...
              << " cores // Avg: " << std::setw(5)
              << (avg_cpu_usage / 100.) * this->numProcessors << std::endl;
    std::cout << "CPU Load         :: " << std::setfill(' ') << std::setw(6)
              << cpu_percent << " %     // Avg: " << std::setw(5)
              << avg_cpu_usage << std::endl;
    std::cout << "RAM Allocation   :: " << std::setfill(' ') << std::setw(6)
              << resident_set / 1000. << " MB    // VSZ: " << vm_usage / 1000.
              << " MB" << std::endl;
}
